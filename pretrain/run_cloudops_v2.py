...
SEED0=int(os.environ.get('SEED0',0))                                           # range(SEED0,NSEED): NSEED is the EXCLUSIVE end (same convention as run_bakeoff_v2)
...
def ffill(a):                                                                  # forward+back fill NaN (Azure traces have missing 5-min bins)
    ...
    m=np.isfinite(a);
    if not m[0]: a[:m.argmax()]=a[m.argmax()]                                   # back-fill leading NaN
    return a

def crps_wql(y, q):                                                            # y:[M], q:[M,9] -> GluonTS mean wQL; NaN targets (imputed points) excluded
    y=np.asarray(y,float); q=np.asarray(q,float); ok=np.isfinite(y)&np.isfinite(q).all(1); y=y[ok]; q=q[ok]
    if len(y)==0: return float('nan')
    den=np.abs(y).sum()+1e-9; tot=0.0
...
def build_train(series, mu, sd, edges):
    ...
    for s in series:
        for d in range(s.shape[0]):                                            # each variate = its own univariate stream
            r=ffill(s[d]); L_=len(r)                                           # impute NaN for model input
...
def eval_crps(m, series, mu, sd, edges):
    ...
    for i in range(0,len(origins),BS):
        ...
        for b,(si,d,ws) in enumerate(batch):
            rf=ffill(series[si][d]); ctx[b]=(rf[ws-SPLIT:ws]-mu)/sd            # imputed context for the model
            tru[b]=series[si][d][ws:ws+PL]                                     # RAW target (NaN steps excluded from CRPS, not scored against imputed)
        ctx=np.concatenate([ctx,np.zeros((len(batch),PL),np.float32)],1)       # pad to train length N=SPLIT+PL; causal mask keeps hid[:,SPLIT-1] pad-free, and mask/pos-bias/1-over-N stay train-consistent
        z=torch.tensor(ctx).to(dev); c=torch.clamp(torch.bucketize(torch.tensor(ctx*sd+mu).to(dev),edg),0,KC-1)
        t=torch.zeros(len(batch),N,dtype=torch.long,device=dev)
        with torch.autocast('cuda',dtype=torch.bfloat16,enabled=dev=='cuda'):
...
if __name__=='__main__':
    ...
    allr=allr[np.isfinite(allr)]                                              # drop NaN (missing bins) from stats
    ...
    print(f"\n==== {CFG} CRPS (GluonTS mean-wQL), mean+-std over seeds {SEED0}..{NSEED-1} ====",flush=True)
